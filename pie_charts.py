...
def original(file_name):
    ...
    rst=[]
    cv2.imshow('image',img)
    messagebox.showinfo("Info", "First select center then select and then all other points clockwise")
...
def export_plot_To_Excel():
    workbook = xw.Workbook("out_pie.xlsx")
